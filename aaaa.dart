...
Future<void> registerNotification(GoRouter router) async {
  ...
  if (Platform.isAndroid && await isHmsAvailable()) {
   ...
   await  hw.Push.registerBackgroundMessageHandler(
      registerHuaweiBackground,
    );
    hw.Push.onNotificationOpenedApp.listen((payload) {
      var data = payload['extras']['notification']['data'] ?? {};
  ...
  try {
    ...
    if (payload.isNotEmpty) {
      hw.Push.localNotification(
        <String, dynamic>{
          hw.HMSLocalNotificationAttr.TITLE: payload['title'] ?? 'SGX Notification',
          hw.HMSLocalNotificationAttr.MESSAGE: payload['message'] ?? '',
          hw.HMSLocalNotificationAttr.DATA : payload,
        },
      );
    }
  } catch (e) {
