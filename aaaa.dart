  ...
  _onSignInClick(SocialLoginProvider provider) async {
    ...
    try {
      ...
      if (shouldMergeFavourites == true) {
        await _favouriteService.mergedFavourites(
    ...
    return showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: isAccountChange
            ? Text(widget.content.changeAccount)
            : Text(widget.content.confirmSignOut),
        ...
        content: Column(
          ...
          children: [
            isAccountChange
                ? Text(widget.content.changeAccountMessage)
                : Text(widget.content.signOutMessage),
          ],
        ),
        actions: [
          TextButton(
            style: falconVars.mediumBtn,
            child: Text(widget.content.cancel),
            onPressed: () => Navigator.of(ctx).pop(false),
          ),
          TextButton(
            style: falconVars.mediumBtn,
            child: Text(widget.content.confirm),
  ...
  Future<bool?> _showFavouritesMergeDialog() {
    ...
    return showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(falconVars.size4)),
        title: Text(widget.content.mergeFavouritesData),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [Text(widget.content.favouritesMergeMessage)],
        ),
        actions: [
          TextButton(
            style: falconVars.mediumBtn,
            onPressed: () => Navigator.of(ctx).pop(true),
            child: Text(widget.content.merge),
          ),
          TextButton(
            style: falconVars.mediumBtn,
            onPressed: () => Navigator.of(ctx).pop(false),
            child: Text(widget.content.discard),
          ),
        ],
      ),
    );
